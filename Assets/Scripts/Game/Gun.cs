...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            Quaternion baseRotation = director.rotation;
            float space = shotgunSpread / (shotgunBullets - 1);
            float startAngle = -shotgunSpread / 2;

            for (int i = 0; i < shotgunBullets; i++)
            {
                float angle = startAngle + (space * i);
                Quaternion bulletRotation = baseRotation * Quaternion.Euler(0.0f, 0.0f, angle);
